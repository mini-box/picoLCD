...
WRAPPER="$INSTALL_PATH/picoLCDGraphic/picolcd-wrapper.sh $INSTALL_PATH lcd4linux.conf.rss"
...
then
    ...
    $INSTALL_PATH/picoLCDGraphic/gksu  --message "picoLCD RSS Viewer needs to detach from the kernel HID driver to function correctly" "$WRAPPER"
else
    killall -9 lcd4linux
    $INSTALL_PATH/lcd4linux -f $INSTALL_PATH/picoLCDGraphic/Configs/lcd4linux.conf.rss -q
fi
